...
def main(node_noise_variance : float, training_split='cv'):
    ...
    if len(sys.argv) == 4:
        ...
        if str(sys.argv[3]) == "null":
            ...
            model = Hybrid_1g12_self_edges(input_dim = 88, noise_variance = node_noise_variance, GAT_heads=4, GAT_fill_value=torch.Tensor([0,0,0,0,0,0,1]))
    elif len(sys.argv) == 5:
        if str(sys.argv[3]) == "mean":
        ...
        if str(sys.argv[3]) == "null":
            ...
            model = Hybrid_1g12_self_edges_transformer_style(input_dim = 88, noise_variance = node_noise_variance, GAT_heads=4, GAT_fill_value=torch.Tensor([0,0,0,0,0,0,1]))
    elif len(sys.argv) < 4:
