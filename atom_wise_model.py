...
model = GATModel(input_dim=40, output_dim=2).to(device)
...
for epoch in range(num_epochs):
    ...
    for batch in train_dataloader:
        ...
        bl = training_sample_loss/len(train_sampler)
        ba = training_sample_acc/len(train_sampler)
        bm = training_sample_mcc/len(train_sampler)
    ...
    with torch.no_grad():
        ...
        for batch in val_dataloader:
            ...
            writer.add_scalar('Batch_Acc/MCC',  bm,  val_batch_num)
            val_batch_num += 1
