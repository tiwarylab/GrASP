...
def process_helper( processed_dir, raw_path, i, cutoff=4):
        try:
            arr = np.load(raw_path, allow_pickle=True)
        except Exception as e:
            print("Error loading file: {}".format(raw_path), flush=True)
...
class KLIFSData_noisy_nodes(Dataset):
    def __init__(self, root, num_cpus, cutoff=4, force_process=False):
        self.cutoff=cutoff
        self.force_process=force_process
        self.num_cpus = num_cpus
    ...
    def add_noise(self, data):
            data.x += (0.02**0.5)*torch.randn_like(data.x)
            return data
    ...
    def get(self,idx):
        ...
        try:
            ...
            return self.add_noise(data), self.raw_file_names[idx]
        except Exception as e:
            # print(e) 
            print("Failed Loading File {}/data_{}.pt".format(self.processed_dir,idx), flush=True)
            print(self.cutoff)
            self.process_helper(self.processed_dir, self.raw_paths[idx], idx, cutoff=self.cutoff)
            data = torch.load(os.path.join(self.processed_dir, 'data_{}.pt'.format(idx)))
            return self.add_noise(data), data.x, self.raw_file_names[idx]
